...
public class ReviewUploadDaoImp implements ReviewUploadDao {
	...
	@Override
	public ReviewUploadFileDto rSelectFileById(int uploadId) {
		
		System.out.println("rSelectFileById() 실행");
		return session.selectOne(Namespace+".rSelectFileById", uploadId);
	}
	...
	public List<ReviewUploadFileDto> rSelectFileByReviewId(int reviewId) {
		...
		return session.selectList(Namespace+".rSelectFileByRestId", reviewId);
	}
